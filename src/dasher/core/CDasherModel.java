...
import dasher.events.CEditContextEvent;
...
//	protected CDasherGameMode m_GameMode;
    ...
    public CDasherModel(CEventHandler EventHandler, CSettingsStore SettingsStore, CDasherInterfaceBase DashIface, CAlphIO AlphIO) {
        ...
        m_dMaxRate = getSetting(Elp_parameters.LP_MAX_BITRATE) / 100.0;
        ...
        // TODO: Think about having 'prefered' values here, which getSetting
        // retrieved by DasherInterfaceBase and used to setSetting parameters

        // TODO: We might getSetting a different alphabet to the one we asked for -
        ...
        CAlphIO.AlphInfo oAlphInfo = AlphIO.GetInfo(getSetting(Esp_parameters.SP_ALPHABET_ID));
        m_cAlphabet = new CAlphabet(oAlphInfo);

        setSetting(Esp_parameters.SP_TRAIN_FILE, m_cAlphabet.GetTrainingFile());
        setSetting(Esp_parameters.SP_DEFAULT_COLOUR_ID, m_cAlphabet.GetPalette());

        if (getSetting(Elp_parameters.LP_ORIENTATION) == Opts.AlphabetDefault) {
        ...
        switch ((int) getSetting(Elp_parameters.LP_LANGUAGE_MODEL_ID)) {
            case 0:
        ...
        int iNormalization = (int) getSetting(Elp_parameters.LP_NORMALIZATION);
        ...
        uniformAdd = getUniformAdd((int) getSetting(Elp_parameters.LP_NORMALIZATION));

        ControlSpace = getControlSpace((int) getSetting(Elp_parameters.LP_NORMALIZATION));
    ...
    public void HandleEvent(CEvent Event) {
        ...
        if (Event.m_iEventType == 1) {
            ...
            if (Evt.m_iParameter == Elp_parameters.LP_MAX_BITRATE || Evt.m_iParameter == Elp_parameters.LP_BOOSTFACTOR || Evt.m_iParameter == Elp_parameters.LP_SPEED_DIVISOR) {
                long bitrateTimesBoost = getSetting(Elp_parameters.LP_MAX_BITRATE) * getSetting(Elp_parameters.LP_BOOSTFACTOR);
                m_dMaxRate = ((double) bitrateTimesBoost) / 100.0 / ((double) getSetting(Elp_parameters.LP_SPEED_DIVISOR));
                m_fr.SetMaxBitrate(m_dMaxRate);
    ...
    protected void Make_root(CDasherNode whichchild) //	find a new root node
    {
        ...
        m_Rootmax = m_Rootmin + (range * m_Root.Hbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);
        m_Rootmin = m_Rootmin + (range * m_Root.Lbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);

        long iTargetRange = m_iTargetMax - m_iTargetMin;
        m_iTargetMax = m_iTargetMin + (iTargetRange * m_Root.Hbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);
        m_iTargetMin = m_iTargetMin + (iTargetRange * m_Root.Lbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);
    }
    ...
    protected void Reparent_root(long lower, long upper) {
        ...
        m_Rootmax = m_Rootmax + (((getSetting(Elp_parameters.LP_NORMALIZATION) - upper)) * iRootWidth / iWidth);

        m_Rootmin = m_Rootmin - ((lower) * iRootWidth / iWidth);

        m_iTargetMax = m_iTargetMax + (((getSetting(Elp_parameters.LP_NORMALIZATION) - upper)) * iTargetWidth / iWidth);
    ...
    protected CDasherNode Get_node_under_crosshair() {
        if (getSetting(Ebp_parameters.BP_DELAY_VIEW)) {
            return m_Root.Get_node_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_iTargetMin, m_iTargetMax, getSetting(Elp_parameters.LP_OX), getSetting(Elp_parameters.LP_OY));
        } else {
            return m_Root.Get_node_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_Rootmin, m_Rootmax, getSetting(Elp_parameters.LP_OX), getSetting(Elp_parameters.LP_OY));
        }
    }
    ...
    protected CDasherNode Get_node_under_mouse(long Mousex, long Mousey) {
        if (getSetting(Ebp_parameters.BP_DELAY_VIEW)) {
            return m_Root.Get_node_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_iTargetMin, m_iTargetMax, Mousex, Mousey);
        } else {
            return m_Root.Get_node_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_Rootmin, m_Rootmax, Mousex, Mousey);
        }
    }
...
/////////////////////////////////////////////////////////////////////////////
    ...
    if(getSetting(Ebp_parameters.BP_DELAY_VIEW))
    m_Root.Get_string_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_iTargetMin, m_iTargetMax, Mousex, Mousey, str);
    else
    m_Root.Get_string_under(getSetting(Elp_parameters.LP_NORMALIZATION), m_Rootmin, m_Rootmax, Mousex, Mousey, str);
    }*/
    // Redundant method.
/////////////////////////////////////////////////////////////////////////////
    /**
    ...
    public void Start() {
        ...
        // FIXME - what if we don't getSetting a reply?
     ...
     * <p>
     * If the given context is empty, the context is actually setSetting
     ...
     *
     * @param sNewContext Context to setSetting
     */
    public void SetContext(String sNewContext) {
        ...
        if (sNewContext.length() == 0) {
            m_Root = m_AlphabetManagerFactory.GetRoot(null, 0, (int) getSetting(Elp_parameters.LP_NORMALIZATION), 0);
            ...
            m_Root = m_AlphabetManagerFactory.GetRoot(null, 0, (int) getSetting(Elp_parameters.LP_NORMALIZATION), iRootSymbol);
        ...
        double dFraction = (1 - (1 - m_Root.MostProbableChild() / (double) (getSetting(Elp_parameters.LP_NORMALIZATION))) / 2.0);

        int iWidth = ((int) ((getSetting(Elp_parameters.LP_MAX_Y) / (2.0 * dFraction))));

        m_Rootmin = getSetting(Elp_parameters.LP_MAX_Y) / 2 - iWidth / 2;
        m_Rootmax = getSetting(Elp_parameters.LP_MAX_Y) / 2 + iWidth / 2;
    ...
    protected GNRCReturn Get_new_root_coords(long Mousex, long Mousey) {
        ...
        long iMaxY = (getSetting(Elp_parameters.LP_MAX_Y));
        long iOX = (getSetting(Elp_parameters.LP_OX));
        long iOY = (getSetting(Elp_parameters.LP_OY));
        ...
        // iSteps is the number of update steps we need to getSetting the point
        ...
        retval.iNewMin = (((m_Rootmin - iTargetMin) * getSetting(Elp_parameters.LP_MAX_Y)) / (iTargetMax - iTargetMin));
        retval.iNewMax = (((m_Rootmax - iTargetMax) * getSetting(Elp_parameters.LP_MAX_Y)) / (iTargetMax - iTargetMin) + getSetting(Elp_parameters.LP_MAX_Y));
        return retval;
    }
...
//		Clear out parameters that might getSetting passed in to track user activity
        if (pAdded != null) {
            pAdded.clear();
        }

        if (getSetting(Ebp_parameters.BP_DASHER_PAUSED) && (m_deGotoQueue.size() == 0)) {
            return false;
        }
...
//			works out next viewpoint
             ...
             * return type to getSetting data out of the function.
             */
            ...
            if (getSetting(Ebp_parameters.BP_OLD_STYLE_PUSH)) {
...
//				push at a random node on the RHS
     ...
     * <p>
     * The nodes' Seen flag is also setSetting, preventing repeated
    ...
    protected void RecursiveOutput(CDasherNode Node, ArrayList<CSymbolProb> pAdded) {
        ...
        Node.Seen(true);
        Node.m_NodeManager.Output(Node, pAdded, (int) getSetting(Elp_parameters.LP_NORMALIZATION));
    }
...
//	public PNGCReturn Plan_new_goto_coords(int iRxnew, long mousey)
    ...
    m_Stepnum = (int)getSetting(Elp_parameters.LP_ZOOMSTEPS);
    int iRxnew_dup = iRxnew;
    // note -- iRxnew is the zoom factor  in units of ZOOMDENOM
    retval.o1 = m_Rootmin;
    retval.o2 = m_Rootmax;
    assert(iRxnew > 0);
    if (iRxnew < ZOOMDENOM && m_Rootmax< getSetting(Elp_parameters.LP_MAX_Y) && m_Rootmin>0 ) {
    ...
    long miNewrootzoom=getSetting(Elp_parameters.LP_MAX_Y)/2 ;
     ...
     */
    /*long distance = mousey - (getSetting(Elp_parameters.LP_MAX_Y)/2);
    ...
    protected void NewGoTo(long newRootmin, long newRootmax) {
        ...
        if (newRootmin > getSetting(Elp_parameters.LP_MAX_Y) / 2 - 1) {
            newRootmin = getSetting(Elp_parameters.LP_MAX_Y) / 2 - 1;
        }

        if (newRootmax < getSetting(Elp_parameters.LP_MAX_Y) / 2 + 1) {
            newRootmax = getSetting(Elp_parameters.LP_MAX_Y) / 2 + 1;
        }
        ...
        if (newRootmax < m_Rootmax_max && newRootmin > m_Rootmin_min && (newRootmax - newRootmin) > getSetting(Elp_parameters.LP_MAX_Y) / 4) {
            ...
            // TODO - force a new root to be chosen, so that we getSetting better
     ...
     * <p>
     * If the NewNode has its Seen flag already setSetting, returns
     ...
     * has its seen flag setSetting), calling the AlphabetManager's
...
//		 Total number of symbols
        ...
        long uniform = getSetting(Elp_parameters.LP_UNIFORM);

        if (!getSetting(Ebp_parameters.BP_CONTROL_MODE)) {
            control_space = 0;
    ...
    protected void Recursive_Push_Node(CDasherNode Node, int iDepth) {
        ...
        if (Node.Range() < 0.1 * getSetting(Elp_parameters.LP_NORMALIZATION)) {
            return;
        }
    ...
    public boolean CheckForNewRoot(CDasherView View) {
         ...
         * which I think/hope means we ultimately getSetting a pointer to the same object
         * as is done below.
         */
        ...
        if (alive == 1) {
            ...
            long newy1 = y1 + (range * theone.Lbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);
            long newy2 = y1 + (range * theone.Hbnd()) / (int) getSetting(Elp_parameters.LP_NORMALIZATION);
    ...
    public void ScheduleZoom(long dasherx, long dashery) {
        ...
        int iSteps = (int) (getSetting(Elp_parameters.LP_ZOOMSTEPS) * dCFactor);
