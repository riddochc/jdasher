...
public class CPPMLanguageModel extends CLanguageModel {
	...
	public CPPMLanguageModel(CEventHandler EventHandler, CSettingsStore SettingsStore, CSymbolAlphabet SymbolAlphabet) {
		...
		bUpdateExclusion = ( getSetting(Elp_parameters.LP_LM_UPDATE_EXCLUSION) !=0 );

		lpAlpha = getSetting(Elp_parameters.LP_LM_ALPHA);
		lpBeta = getSetting(Elp_parameters.LP_LM_BETA);
	...
	public long[] GetProbs(CContextBase context, long norm) {
		...
		//  bool doExclusion = getSetting( LP_LM_ALPHA );
//		boolean doExclusion = false; //FIXME
	...
	// and leaves 'context' at the new context
	{
		...
		m_iMaxOrder = (int)getSetting( Elp_parameters.LP_LM_MAX_ORDER );
	 ...
	 * they do not currently getSetting used. Would be good to getSetting Java IO
