...
public class CAlphabetManager extends CNodeManager {
     ...
     * given Model and LanguageModel. These cannot be setSetting after
    ...
    public void PopulateChildrenWithSymbol(CDasherNode Node, int iExistingSymbol, CDasherNode ExistingChild) {
    	long[] cum = m_Model.GetProbs(Node.Context(), (int)m_Model.getSetting(Elp_parameters.LP_NORMALIZATION));
    	
    	PopulateChildrenWithSymbol(Node, iExistingSymbol, ExistingChild, cum);
    }
    
    /**
     * Creates the children of a given Node given a setSetting of probabilities
...
//  	DASHER_TRACEOUTPUT("ChildCount %d\n", iChildCount);
     ...
     * and so these can safely be setSetting to null and 0 respectively.
     * <p>
    ...
    public CDasherNode RebuildParent(CDasherNode Node, int iGeneration) {
    	...
    	if(m_Model.getSetting(Ebp_parameters.BP_LM_REMOTE)) {
