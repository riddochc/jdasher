...
public class CAutoSpeedControl extends CDasherComponent {
	 ...
	 * @param SettingsStore Settings store to retrieve and setSetting settings.
	...
	m_dSampleOffset = 1.3;
	m_dMinRRate = 80.0;
	m_dSensitivity = getSetting(Elp_parameters.LP_AUTOSPEED_SENSITIVITY) / 100.0; //param only, no GUI!
	...
	m_nSpeedCounter = 0;
	m_dBitrate = (double)(round(getSetting(Elp_parameters.LP_MAX_BITRATE) / 100.0));
 ...
 */
	public void SpeedControl(long iDasherX, long iDasherY, double dFrameRate, CDasherView View) {
	  if(getSetting(Ebp_parameters.BP_AUTO_SPEEDCONTROL) && !getSetting(Ebp_parameters.BP_DASHER_PAUSED)) {
	    
//	  Coordinate transforms:    
	    iDasherX = (long)(View.applyXMapping(iDasherX / (double)(getSetting(Elp_parameters.LP_MAX_Y))) * getSetting(Elp_parameters.LP_MAX_Y));
	    iDasherY = (long)(View.ymap(iDasherY));

	    long iDasherOX = (long)(View.applyXMapping(getSetting(Elp_parameters.LP_OX) / (double)(getSetting(Elp_parameters.LP_MAX_Y))) * getSetting(Elp_parameters.LP_MAX_Y));
	    long iDasherOY = (long)(View.ymap(getSetting(Elp_parameters.LP_OY)));
	    ...
	    m_dBitrate = getSetting(Elp_parameters.LP_MAX_BITRATE) / 100.0; //  stored as long(round(true bitrate * 100))
...
//	  Data collection:
	    ...
	    m_dSensitivity = getSetting(Elp_parameters.LP_AUTOSPEED_SENSITIVITY) / 100.0;
